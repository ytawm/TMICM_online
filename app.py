    ...
    if request.method == 'POST':
        if 'button1' in request.form:
            ...
            if f.filename is not '':
                ...
                img_path = f.filename.replace(' ','_').replace('(', '').replace(')', '')
                shutil.copy(img_path, 'static')
                pred_label, prediction = run_app(img_path)
                img_paths = img_path
                ...
                return render_template('index.html', prediction=['预测结果：', '亲，你确定你上传图像了吗？', '', ''], show_email=show_email)
        ...
        if 'button2' in request.form:
            ...
            return render_template('index.html', prediction=["预测结果：", str(email_content) + "。该邮件发送成功...", "", str(email_content)], show_email=show_email)
